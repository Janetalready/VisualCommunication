...
def cal_reward(one_hot, probs, gt, opt, cnt, mask):
    ...
    if np.any(np.isnan(a.data.clone().cpu().numpy())):
        pdb.set_trace()
    return - ((rewards_no_grad)*(opt.discount**cnt)
                        * masked_log_proba_receiver * mask), rewards_no_grad*(opt.discount**cnt)
...
class Players(nn.Module):
    ...
    def forward(self, images_vectors, images_vectors_receiver,
        ...
        canvas0 = torch.zeros([opt.batch_size, 3, self.width, self.width], requires_grad=False).to(self.device)
        total_loss = torch.zeros(opt.batch_size).cuda()
        ...
        one_hot_output, receiver_probs = receiver_action_retrieve(self.receiver,
                                                                  images_vectors_receiver, canvas0, canvas, opt,
                                                                  train)
        _, amax = one_hot_output.max(dim=1)

        step_loss, step_reward = cal_reward(one_hot_output, receiver_probs, gt, opt, 0, mask)
        ...
        if not train:
            ...
            res[mask_, step_num] = canvas.cpu().numpy()[mask_]
        ...
        return total_loss, res, precision, cnt.cpu().numpy().astype(int), choice, total_reward
